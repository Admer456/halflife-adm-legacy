...
libraries, 
which are not shipped with Visual C++ 2010 Express / Windows SDK
v7.0A.
...
Please not that qrad.exe requires additional files lights.rad and valve.rad
 side-by-side. 
You can find those files in the Half-Life SDK in the Hammer
tools folder.
...
- It doesn't seem to work anymore, I tried with a recent HLDS beta installation
  from steamcmd 
(make sure to set the hardcoded path in ServerCtrlDlg.cpp line
...
/MD or /MDd instead of /MT or /MTd now. 
The reason is that the MFC from the
Windows Driver Kit won't compile in static mode (missing resource files i.e.).

This means you'll have to redistribute the runtime and MFC DLLs along with serverctrl in case you install it on another system.
...
Properties. 
Select Configuration: All Configurations, then select
 Configuration -> VC++ Directories in the tree. 
Adjust the Include and

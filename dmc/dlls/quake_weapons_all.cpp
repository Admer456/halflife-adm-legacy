...
void CBasePlayer::W_FireAxe()
{
	...
	if (pEntity && pEntity->pev->takedamage)
	{
		...
		std::string szEntityClassname = STRING(pEntity->pev->classname);
		if (szEntityClassname[0] != 'f')
		{
			if (g_pGameRules->PlayerRelationship(this, pEntity) != GR_TEAMMATE)
				SpawnBlood(vecOrg, BLOOD_COLOR_RED, iDmg * 4); // Make a lot of Blood!

			ALERT(at_aiconsole, "\nThis entity is not a func_ entity, so it can bleed (classname %s)", STRING(pEntity->pev->classname));
		}
#endif
	}
}

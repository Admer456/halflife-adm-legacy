...
BOOL CBaseMonster :: FInViewCone ( CBaseEntity *pEntity )
{
	...
	vec2LOS = ( pEntity->pev->origin - pev->origin ).To2D();
	vec2LOS = vec2LOS.Normalized();

	flDot = DotProduct (vec2LOS , gpGlobals->v_forward.To2D() );
...
BOOL CBaseMonster :: FInViewCone ( Vector *pOrigin )
{
	...
	vec2LOS = ( *pOrigin - pev->origin ).To2D();
	vec2LOS = vec2LOS.Normalized();
...
BOOL CBaseEntity :: FVisible ( CBaseEntity *pEntity )
{
	...
	fNovis = FStrEq(STRING(tr.pHit->v.classname), "func_novis");

	CBaseEntity *pList[64];

	Vector delta = Vector(4, 4, 4);

	// Find only brush entities in box
	int count = UTIL_EntitiesInBox(pList, 64, pev->origin - delta, pev->origin + delta, FL_WORLDBRUSH);
	for (int i = 0; i < count; i++)
	{
		if ( FStrEq( STRING( pList[i]->pev->classname ), "func_novis" ) )
			fNovis = FALSE; // we are inside of a func_novis, so this is false xd
	}

	developer = CVAR_GET_FLOAT("developer");

	if (developer == 2)
		ALERT(at_console, "\nI see a %s", STRING(tr.pHit->v.classname));

	if (tr.flFraction != 1.0 || fNovis)
	{
		if (developer == 69)

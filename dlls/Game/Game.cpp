...
// First gamemode defines the base game, AdmSrc, or HL, or CS, second gamemode defines whether it's SP or MP, and the third one defines the actual game mode, e.g. deathmatch
cvar_t	adm_gamemode = { "adm_gamemode", "aug", FCVAR_SERVER };	// Reserved for minigames, this will be used to change weapon models, NPC models, and so on, according to this CVAR
cvar_t	adm_gamemode2 = { "adm_gamemode2", "sp", FCVAR_SERVER }; // sp - single player, mp - multi player
cvar_t	adm_gamemode3 = { "adm_gamemode3", "campaign", FCVAR_SERVER }; // campaign is usually for SP, deathmatch is for MP, there will also be tdeathmatch, and whatever one comes up with
cvar_t	adm_subtitles = { "adm_subtitles", "1", FCVAR_SERVER }; // Self-explanatory, and besides, you know for sure I'll be adding these in :P

// Gamemode 1 CVAR
// aug - An unknown game (a.k.a. Admer The Game, my first mod)
// hl - Half-Life, will use default HL HUD, HL weapons, models, sprites and so on (TBI)
// cs - Counter-Strike? (TBI)

// Gamemode 2 CVAR
// sp - Single player. Can be campaign, or minigame. Hmmmmm, so you could technically play through HL in this mod. LOL
// mp - Multiplayer. Can be deathmatch, TDM, co-op and so on.
...
// FYI TBI stands for To Be Implemented

/*
// AdmSrc - customisation cvars - edit: moved to clientside

cvar_t	adm_hud_r = { "adm_hud_r", "255", FCVAR_SERVER };
cvar_t	adm_hud_g = { "adm_hud_g", "255", FCVAR_SERVER };
cvar_t	adm_hud_b = { "adm_hud_b", "255", FCVAR_SERVER };*/
...
cvar_t	falldamage	= {"mp_falldamage","0", FCVAR_SERVER }; // I assume 0 is 10 HP, and 1 is realistic. HMMM >:3
cvar_t	weaponstay	= {"mp_weaponstay","0", FCVAR_SERVER };
cvar_t	forcerespawn= {"mp_forcerespawn","1", FCVAR_SERVER };
cvar_t	flashlight	= {"mp_flashlight","1", FCVAR_SERVER };	// AdmSrc - changed to 1, 0 by default
...
void GameDLLInit( void )
{
	...
	// AdmSrc stuff
	CVAR_REGISTER(&adm_gamemode);
	CVAR_REGISTER(&adm_gamemode2);
	CVAR_REGISTER(&adm_gamemode3);

	CVAR_REGISTER(&adm_subtitles);

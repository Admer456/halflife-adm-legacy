...
void ClientCommand( edict_t *pEntity )
{
	...
	else if (FStrEq(pcmd, "+key_adm_k2"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(adm_k2, true);

	else if (FStrEq(pcmd, "-key_adm_k2"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(adm_k2, false);

	else if (FStrEq(pcmd, "+key_sprint"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(sprint, true);

	else if (FStrEq(pcmd, "-key_sprint"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(sprint, false);

	// ADMSRC SOUND SYSTEM COMMS

//	else if (FStrEq(pcmd, "admsi_play"))
//	{
//		MESSAGE_BEGIN(MSG_ONE, gmsgADMSIPlay, NULL, ENT(pev));
//			WRITE_STRING((char *)CMD_ARGV(1));
//		MESSAGE_END();
//	}

	else if ( FStrEq( pcmd, "fss_playmusic" ) )
	{
//		int iStringTableIndex = atoi( CMD_ARGV( 1 ) );
		MESSAGE_BEGIN( MSG_ONE, gmsgFSSMusic, NULL, ENT( pev ) );
			WRITE_STRING( (char *)CMD_ARGV( 1 ) );
		MESSAGE_END();
	}
	...
	else if (FStrEq(pcmd, "+key_vcltc"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_cltc, true);

	else if (FStrEq(pcmd, "-key_vcltc"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_cltc, false);

	else if (FStrEq(pcmd, "+key_vup"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_up, true);

	else if (FStrEq(pcmd, "-key_vup"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_up, false);

	else if (FStrEq(pcmd, "+key_vdn"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_dn, true);

	else if (FStrEq(pcmd, "-key_vdn"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_dn, false);

	else if (FStrEq(pcmd, "+key_vllt"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_llt, true);

	else if (FStrEq(pcmd, "-key_vllt"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_llt, false);

	else if (FStrEq(pcmd, "+key_vlrt"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_lrt, true);

	else if (FStrEq(pcmd, "-key_vlrt"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_lrt, false);

	// VEHICLE   NPC   COMMANDS

	else if (FStrEq(pcmd, "+key_vjmp"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_jump, true);

	else if (FStrEq(pcmd, "-key_vjmp"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_jump, false);

	// VEHICLE GENERAL COMMANDS

	else if (FStrEq(pcmd, "+key_vunuse"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_unuse, true);

	else if (FStrEq(pcmd, "-key_vunuse"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_unuse, false);

	else if (FStrEq(pcmd, "+key_vstart"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_startstop, true);

	else if (FStrEq(pcmd, "-key_vstart"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_startstop, false);
	
	else if (FStrEq(pcmd, "+key_vseats"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_seatswitch, true);
	
	else if (FStrEq(pcmd, "-key_vseats"))
		GetClassPtr((CBasePlayer *)pev)->SetKeyButton(vehicle_seatswitch, false);
...
// player pain sounds
	...
	PRECACHE_GENERIC( "soundcache.txt" );

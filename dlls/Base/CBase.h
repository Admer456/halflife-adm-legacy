...
public:
	...
	//		AdmSrc-specific code
	// Get and Set are required so trigger_valueop can function.
	// SOHL's changevalue creates a KeyValueData structure, and basically it uses the regular
	// KeyValue method to do its thing. But that doesn't work, and it needs quite a few lines.
	// So, I decided to make it a bit more comfortable, and cool, albeit with a small drawback:
	//
	// GetKeyValue:
	// - if input string matches any of the pev (CBaseEntity's) keyvalues, do that
	// - else, call GetKeyValueBase
	// - - if input string matches any of e.g. CBaseDelay's keyvalues, do that
	// - - else, call GetKeyValueCustom
	// - - - if input string matches any of e.g. env_viewsway's keyvalues, do that
	// - - - else, return "null"
	//
	// The problem is, you'd have to add a block of if - else if - else statements for each
	// entity and base you're writing. But I suppose that's not too much of a task.

	// GetKeyValue, used by trigger_valueop, but other entities may also utilise this very fine function
	// GetKeyValueBase essentially is meant for bases like CBaseDelay etc., while CBaseEntity has its own GetKeyValue
	// GetKeyValueCustom is meant for specific entities e.g. env_viewsway that have really custom keyvalues
	virtual void	GetKeyValue			(char &szTheValue, char szTargetKeyName[64]); 
	virtual void	GetKeyValueBase		(char &szTheValue, char szTargetKeyName[64]) { GetKeyValueCustom(szTheValue, szTargetKeyName); }
	virtual void	GetKeyValueCustom	(char &szTheValue, char szTargetKeyName[64]) { sprintf(&szTheValue, "null (no matching KV)"); return; }

	// Same as GetKeyValue/Base/Custom, except it sets it
	virtual void	SetKeyValue			(char szTargetKeyName[64], char szTheValue[64]);
...
//	virtual void	SetActivator( CBaseEntity *pActivator ) {}
	...
	void EXPORT SUB_NullThink(float delay) { /*Think();*/ pev->nextthink = pev->ltime + delay; }
...
public:
	void				KeyValue( KeyValueData *pkvd );
	void			GetKeyValueBase(char &szTheValue, char szTargetKeyName[64]);
	void			SetKeyValueBase(char szTargetKeyName[64], char szTheValue[64]);

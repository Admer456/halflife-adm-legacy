...
void CWorld :: KeyValue( KeyValueData *pkvd )
{
	...
	{
		pev->targetname = MAKE_STRING(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	...
	else if ( FStrEq(pkvd->szKeyName, "physics") )
	{
		if (atoi(pkvd->szValue))
		{
			m_fHasPhysics = TRUE;
...
{
	if (FStrEq(szTargetKeyName, "skyname"))
		sprintf(&szTheValue, "%s", CVAR_GET_STRING("sv_skyname"));

	else if (FStrEq(szTargetKeyName, "sounds"))
		sprintf(&szTheValue, "%i", gpGlobals->cdAudioTrack);

	else if (FStrEq(szTargetKeyName, "WaveHeight"))
		sprintf(&szTheValue, "%f", (CVAR_GET_FLOAT("sv_wateramp") * 8));

	else if (FStrEq(szTargetKeyName, "MaxRange"))
		sprintf(&szTheValue, "%f", pev->speed);

	else if (FStrEq(szTargetKeyName, "chaptertitle"))
		sprintf(&szTheValue, "%s", STRING(pev->netname));

	else if (FStrEq(szTargetKeyName, "startdark"))
		sprintf(&szTheValue, "%i", (pev->spawnflags & SF_WORLD_DARK)); // It should return a number bigger than zero, IF dark is set in the spawnflags :)

	else if (FStrEq(szTargetKeyName, "newunit"))
		sprintf(&szTheValue, "%f", CVAR_GET_FLOAT("sv_newunit"));

	else if (FStrEq(szTargetKeyName, "gametitle"))
		sprintf(&szTheValue, "%i", (pev->spawnflags & SF_WORLD_TITLE));

	else if (FStrEq(szTargetKeyName, "mapteams"))
		sprintf(&szTheValue, "%s", STRING(pev->team));

	else if (FStrEq(szTargetKeyName, "defaultteam"))
		sprintf(&szTheValue, "%i", (pev->spawnflags & SF_WORLD_FORCETEAM));

	else
		sprintf(&szTheValue, "null (no matching KV)");
}

void CWorld::SetKeyValueCustom(char szTargetKeyName[64], char szTheValue[64])
{
	if (FStrEq(szTargetKeyName, "skyname"))
		CVAR_SET_STRING("sv_skyname", szTheValue);

	else if (FStrEq(szTargetKeyName, "sounds"))
		gpGlobals->cdAudioTrack = atoi(szTheValue);

	else if (FStrEq(szTargetKeyName, "WaveHeight"))
		CVAR_SET_FLOAT("sv_wateramp", (atof(szTheValue) * 0.125));

	else if (FStrEq(szTargetKeyName, "MaxRange"))
		pev->speed = atof(szTheValue);
	...
	else if (FStrEq(szTargetKeyName, "startdark"))
	{
		if (atoi(szTheValue))
			...
			pev->spawnflags ^= SF_WORLD_DARK; // This is some voodoo magic right there
	}

	else if (FStrEq(szTargetKeyName, "newunit"))
		CVAR_SET_FLOAT("sv_newunit", atof(szTheValue));
	...
	else if (FStrEq(szTargetKeyName, "gametitle"))
	{
		if (atoi(szTheValue))
			...
			pev->spawnflags ^= SF_WORLD_TITLE; // More magic, MAGIC SON, MAGIC
	}

	else if (FStrEq(szTargetKeyName, "mapteams"))
		pev->team = ALLOC_STRING(szTheValue);
	...
	else if (FStrEq(szTargetKeyName, "defaultteam"))
	{
		if (atoi(szTheValue))
			...
			pev->spawnflags ^= SF_WORLD_FORCETEAM; // More magic, MAGIC SON, MAGIC
	}

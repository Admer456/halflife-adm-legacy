...
// Keeps clutter down a bit, when writing key-value pairs¸ -- NOTE: Obsolete API
...
// Compares current keyvalue's name, with our given name
#define KeyvalueToken(keyvalue_name) FStrEq(pkvd->szKeyName, #keyvalue_name)

// Converts given keyvalue to an int
#define KeyvalueToInt(member_var)		\
{										\
	member_var = atoi(pkvd->szValue);	\
	pkvd->fHandled = TRUE;				\
}

// Converts given keyvalue to an float
#define KeyvalueToFloat(member_var)		\
{										\
	member_var = atof(pkvd->szValue);	\
	pkvd->fHandled = TRUE;				\
}

// Converts given keyvalue to a string
#define KeyvalueToString(member_var)			\
{												\
	member_var = ALLOC_STRING(pkvd->szValue);	\
	pkvd->fHandled = TRUE;						\
} 

// Converts a given keyvalue to a vector
...
#define KeyvaluesFromBase(baseclass) baseclass::KeyValue(pkvd);
...
extern void			UTIL_SetSizeAuto		(entvars_t* pev); // found at http://articles.thewavelength.net/214/
...
extern void			UTIL_ScreenShakeAir		( const Vector &center, float amplitude, float frequency, float duration, float radius );
extern void			UTIL_ScreenShakeAll		( const Vector &center, float amplitude, float frequency, float duration );
extern void			UTIL_ScreenShakeAllAir	( const Vector &center, float amplitude, float frequency, float duration );
...
inline bool			FBitCheck(unsigned char input, unsigned char bit) { return input & bit; };

// Draws a debug line, using the Efx API. You cannot call more than one of these per one entity's think cycle
// dir 0 = forward, dir 1 = right, dir 2 = up
extern void			UTIL_DbgDrawLine(float radius, Vector origin, Vector angles, int dir);

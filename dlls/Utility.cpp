...
CBaseEntity *UTIL_EntityByIndex(int entIndex, int maxIndex)
{
	CBaseEntity *pEntity = NULL;

	if (entIndex > 0 && entIndex <= maxIndex)
	{
		edict_t *pEntityEdict = INDEXENT(entIndex);
		if (pEntityEdict)
		{
...
void UTIL_ScreenShakeAir( const Vector &center, float amplitude, float frequency, float duration, float radius )
{
	int			i;
	float		localAmplitude;
	ScreenShake	shake;
	...
	if (!pPlayer)
			continue;
//		LMAO, this was easier than I thought! :D
...
void UTIL_ScreenShakeAllAir(const Vector &center, float amplitude, float frequency, float duration)
{
	UTIL_ScreenShakeAir(center, amplitude, frequency, duration, 0);
}
...
void UTIL_DbgDrawLine(float radius, Vector origin, Vector angles, int dir)
{
	Vector end, vecForward, vecRight, vecUp;
	UTIL_MakeVectorsPrivate(angles, vecForward, vecRight, vecUp);

	switch (dir)
	{
	case 0:
		end = radius * vecForward;
		break;
...
{
	UTIL_Deg2Rad(angles.x);
	UTIL_Deg2Rad(angles.y);
	UTIL_Deg2Rad(angles.z);

	Vector vecDest;
	vecDest.x = radius * cos(angles.x) * cos(angles.y);
	vecDest.y = radius * cos(angles.x) * sin(angles.y);
	vecDest.z = radius * sin(angles.x);

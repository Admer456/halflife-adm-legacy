...
{
	if (useType == USE_OFF)
		ALERT( at_aiconsole, "func_wall, useType is USE_OFF(0)" );

	if (useType == USE_ON)
		ALERT( at_aiconsole, "func_wall, useType is USE_ON(1)" );

	if (useType == USE_SET)
		ALERT( at_aiconsole, "func_wall, useType is USE_SET(2)" ) ;

	if (useType == USE_TOGGLE)
		ALERT( at_aiconsole, "func_wall, useType is USE_TOGGLE(3)" );
...
/*	pev->avelocity.x = 30;
	...
	ALERT(at_console, "rotating,angles: %f %f %f\navelocity: %f %f %f",
		pev->angles.x,
		pev->angles.y,
		pev->angles.z,
